"""
htm_velocity/htm_velocity_collect_results.py
Aggregate all completed HTM-Velocity runs into a leaderboard.

Usage (from project root):
  python htm_velocity/htm_velocity_collect_results.py [--top 20]

Reads:   hv_results/hv*.json
Writes:  hv_results/leaderboard.txt
         hv_results/leaderboard.csv
"""
...
def load_results() -> list:
    records = []
    for fpath in sorted(glob.glob(os.path.join(RESULTS_DIR, "hv*.json"))):
        try:
...
def format_row(rank: int, r: dict) -> str:
    ...
    # Poly error encoder string
    poly_str = (
        f"pm={cfg.get('poly_mean_enc_bits','?')}w{cfg.get('poly_mean_enc_w','?')} "
        f"pd={cfg.get('poly_med_enc_bits','?')}w{cfg.get('poly_med_enc_w','?')} "
        f"ps={cfg.get('poly_std_enc_bits','?')}w{cfg.get('poly_std_enc_w','?')}"
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Collect HTM-Velocity hyperparameter search results")
    ...
    if not records:
        print(f"No results found in {RESULTS_DIR}/. "
              "Run htm_velocity_train_single.py jobs first.")
        sys.exit(0)

    # Sort: deployable (live_thresh>0) FIRST, then test F1, val BAcc, config_idx
    records.sort(
    ...
    lines  = [sep,
              f"  HTM-Velocity Hyperparameter Search -- {total} completed runs",
              sep, header, "-" * 185]
    ...
    best_lt_str = (f"{best_lt:.4f}"
                   if best_lt is not None and best_lt > 0
                   else "N/A (model not deployable)")
    lines += [
        sep,
        f"  Best config : hv_configs/config_{best.get('config_idx', 0):04d}.json",
        f"  Best model  : {best.get('model_file', '(see hv_models/)')}",
