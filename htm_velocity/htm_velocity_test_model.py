"""
htm_velocity/htm_velocity_test_model.py
Test a trained HTM-Velocity model on multiple evaluation modes.

Modes:
  orig            — original val/test splits from split.pkl
  all_non_train   — all files not in the training set (uses hv_windows_cache if available)
  all_other_files — raw .txt files outside the original splits; parses on-the-fly
                    using parse_file_distance + poly_regressor.pkl

Usage (from project root):
  python htm_velocity/htm_velocity_test_model.py \\
      --model hv_models/<slug>.pkl [--mode all_non_train] [--write_decision_log]
"""
...
from htm_combined_common import apply_detection, parse_file_distance, WARMUP_STEPS
...
STATS_CACHE  = "stats_cache.pkl"
DIST_CACHE   = "dist_cache.pkl"
POLY_MODEL   = "poly_regressor.pkl"
...
def _extract_worker(args):
    filepath, window_size, window_step, ref_dwells, ref_flights, ref_dists, poly_model_path = args
...
def get_scores(file_list, is_bot, model_data, seqs_dict, warmup, al_period):
    ...
    for fp in file_list:
        ...
        tm.reset()
        raw = []
        for stats, key_idx, dwell, flight, dist, poly_mean, poly_med, poly_std in seq:
...
# ──────────────────────────────────────────────────────────────────────────────
# Load sequences from windows cache or velocity cache
# ──────────────────────────────────────────────────────────────────────────────
def _load_seqs_from_cache(file_list, wcache, vel_cache, stats_cache,
    ...
    for fp in file_list:
        if wcache is not None:
            ...
            # Build from velocity_cache or fall back to on-the-fly
            vel_events = vel_cache.get(fp) if vel_cache else None
            if vel_events is None and stats_cache is not None and poly_model is not None:
                events = stats_cache.get(fp)
                if events:
                    tmp = build_velocity_cache_from_stats({fp: events}, poly_model)
                    vel_events = tmp.get(fp)
            if vel_events is None:
                continue
            events = stats_cache.get(fp) if stats_cache else None
            if not events:
                continue
            seq = get_file_velocity_seq(events, vel_events, window_size, window_step,
                                        ref_dwells, ref_flights, ref_dists)
...
def main():
    parser = argparse.ArgumentParser(
        description="Test a trained HTM-Velocity model")
    parser.add_argument("--model", required=True,
                        help="Path to the .pkl model file (hv_models/<slug>.pkl)")
    ...
    print(f"  Config: hv{config_idx:04d}  thresh={best_thresh:.4f}  "
    ...
    if os.path.exists(wcache_path):
        ...
        print(f"Windows cache not found ({wcache_path}); falling back to velocity/stats cache.")
        if os.path.exists(VEL_CACHE):
            print(f"  Loading {VEL_CACHE}")
            with open(VEL_CACHE, 'rb') as fh:
                vel_cache = pickle.load(fh)
        cache_path = STATS_CACHE if os.path.exists(STATS_CACHE) else DIST_CACHE
        if os.path.exists(cache_path):
            print(f"  Loading {cache_path}")
            with open(cache_path, 'rb') as fh:
                stats_cache = pickle.load(fh)
        if vel_cache is None and os.path.exists(args.poly_model):
            print(f"  Loading poly model for on-the-fly poly errors: {args.poly_model}")
            poly_model = joblib.load(args.poly_model)

    def load_seqs(file_list):
        return _load_seqs_from_cache(
            file_list, wcache, vel_cache, stats_cache,
            window_size, window_step, ref_dwells, ref_flights, ref_dists,
    ...
    # ──────────────────────────────────────────────────────────────
        ...
        if not os.path.exists(args.poly_model):
            print(f"ERROR: poly_regressor.pkl not found at '{args.poly_model}'.")
            print("  Required for all_other_files mode to compute poly errors.")
            sys.exit(1)
        ...
        poly_model_path = os.path.abspath(args.poly_model)
        worker_args = [
            (fp, window_size, window_step, ref_dwells, ref_flights, ref_dists, poly_model_path)
